...
def detr_resnet50(pretrained=True, num_classes=7, return_postprocessor=True):
    """
    ...
    if pretrained:
        if torch.cuda.is_available():
            ...
            "./checkpoints/best_detr.pth"
        )
        else:
            checkpoint = torch.load(
            "./checkpoints/best_detr.pth",map_location=torch.device('cpu')
            )
